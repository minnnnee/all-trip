...
export default function Home() {
  ...
  const handleShare = async () => {
    ...
    const text = `${location.name}, ${location.country} ${MONTH_LABEL[month]} ${PL[period]} 여행 정보 — Tripwise`;
    if (navigator.share) {
      await navigator.share({ title: 'Tripwise', text, url: window.location.href }).catch(() => {});
    } else {
    ...
    <div className="flex flex-col min-h-screen bg-slate-100">
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-sm border-b border-slate-200 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span className="font-bold text-slate-800 text-base shrink-0">Tripwise</span>
          {result && (
